...
function SendWarningPopup({ open, onClose, onConfirm }) {
  ...
  const handleConfirm = (e) => {
    ...
    setShowError(false);
    onConfirm(true); // ✅ تأكيد الإرسال فقط
  };
  ...
  return (
    <div className="popup-overlay">
      <div className="popup-box figma-popup-box" dir="rtl">
        ...
        <div className="popup-title figma-popup-title">تنبيه هام</div>
        ...
        {showError && (
          <div className="popup-error figma-popup-error">يجب الموافقة على السياسات للمتابعة</div>
        )}
        <div className="popup-actions figma-popup-actions">
          <button className="popup-cancel figma-popup-cancel" onClick={onClose}>إلغاء</button>
          <button className="popup-confirm figma-popup-confirm" onClick={handleConfirm}>متابعة الإرسال</button>
        </div>
      </div>
    </div>
  );
